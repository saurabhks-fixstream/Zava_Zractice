...
public class VerticalOrderTraversalofaBinaryTree {
    ...
    public static List<List<Integer>> verticalTraversal(TreeNode root) {
        ...
        Map<Integer, Map<Integer, PriorityQueue<Integer>>> map =
                new TreeMap<Integer, Map<Integer, PriorityQueue<Integer>>>();
        verticalTraversalHelper(root, 0, 0, map);
        for (Entry<Integer, Map<Integer, PriorityQueue<Integer>>> entry : map.entrySet()) {
            result.add(new ArrayList<Integer>());
            for (Entry<Integer, PriorityQueue<Integer>> e : entry.getValue().entrySet()) {
    ...
    private static void verticalTraversalHelper(
            TreeNode root, int x, int y, Map<Integer, Map<Integer, PriorityQueue<Integer>>> map) {
        if (root == null) {
            return;
        }
